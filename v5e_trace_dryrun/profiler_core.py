# Auto-generated from llm_profiler_tpu.ipynb cells 1 + 4 + 6
...
def run_profile(
    ...
    def _load_events(logdir_or_file: str):
        ...
        if os.path.isdir(logdir_or_file):
            # prefer the newest profile run
            files = _latest_trace_files(logdir_or_file)
        else:
            if logdir_or_file.endswith(".trace.json.gz") or logdir_or_file.endswith(".json"):
    ...
    for input_len, kv_len in outer:
        ...
        if use_xla_timers:
            ...
            # torch_xla 2.5: no start_trace/stop_trace. Use threaded xp.trace().
            _svc = f"127.0.0.1:{os.environ.get('XLA_PROFILER_PORT', '9012')}"
            # Conservative duration: covers compile + repeat forwards for large shapes.
            _duration_ms = 6000
            _thr = threading.Thread(
                target=lambda: xp.trace(_svc, trace_dir, duration_ms=_duration_ms),
                daemon=True,
            )
            _thr.start()
            time.sleep(0.2)  # let profiler connect
            ...
            torch_xla.sync()
            # wait for threaded trace to finish writing
            _thr.join(timeout=_duration_ms / 1000.0 + 5.0)
